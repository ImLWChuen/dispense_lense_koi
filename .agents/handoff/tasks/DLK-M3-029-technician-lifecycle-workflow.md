...
- `frontend/components/diagnosis/DiagnosticQuestion.tsx`
...
Following code review `DLK-M3-029-review.md`, resolved review findings R1–R12 in full:
...
11. **R11**: Corrected POST-failure plus GET-failure transition in `coordinateWorkflowMutation`. Preserved original mutation error and technician input, set `isRefreshRequired: true`, gated all further workflow mutations, and displayed truthful unconfirmed warning (`UNCONFIRMED_REFRESH_WARNING_MESSAGE = "The action result could not be confirmed, and the latest case state could not be refreshed. Refresh the case before retrying."`). Introduced explicit `refreshWarningKind` state (`"action_saved"` vs `"state_refresh_required"`) across questions, troubleshooting, and verification views so the UI distinguishes committed POST + failed GET from unconfirmed POST + failed GET without false claims. Updated Test 15 Scenario C asserting gating, truthful warning, and GET-only retry clearing all flags with zero POST replay.
12. **R12**: Added `frontend/components/diagnosis/DiagnosticQuestion.tsx` to Allowed paths. Separated active-question locking from persisted completion styling by adding a distinct `disabled` prop used solely to disable option buttons (`disabled={isSubmitting || isRefreshRequired}` while `isAnswered={false}`). Kept green completion styling and check icon exclusively for questions in persisted answer history. Added `deriveQuestionPresentation` helper and Test 16 covering active, submitting/disabled, refresh-required/disabled, and persisted-answered states.
...
- `frontend/lib/diagnostic-workflow-state.ts`: Added pure workflow state module with `deriveQuestionsView`, `deriveTroubleshootingView`, `buildCheckResultPayload`, `formatOutcomeLabel`, `getAvailableLifecycleActions`, lifecycle payload builders, `formatEvidenceSupport`, `resolveActiveCheck`, `shouldShowVerificationBadge`, `applyMutationSuccess`, `applyMutationFailure`, `evaluateFormInputsOnMutation`, `coordinateWorkflowMutation`, `retryWorkflowRefresh`, `WorkflowMutationError`, `RefreshWarningKind`, `UNCONFIRMED_REFRESH_WARNING_MESSAGE`, and `deriveQuestionPresentation`.
- `frontend/scripts/test-diagnostic-workflow-state.mjs`: Added 16-test dependency-free regression test suite covering view mutual exclusion, canonical outcome preservation (`no_blockage`), safe findings across all 6 execution statuses, lifecycle transitions, score formatting, error preservation, input preservation on failure, read-only historical checks, required verification details, production coordinator state lifecycle across all 4 outcomes with gating and GET-only retry, and diagnostic question states separating locking from completion styling.
...
- `frontend/components/diagnosis/DiagnosticQuestion.tsx`: Added `disabled` prop to disable option buttons during submission or refresh-required gating while keeping green completion styling exclusively for persisted answers.
- `frontend/app/(dashboard)/diagnosis/[id]/questions/page.tsx`: Wired `deriveQuestionsView`, `coordinateWorkflowMutation`, `retryWorkflowRefresh`, dedicated error state with retry, distinct refresh warning banners distinguishing "Action Saved" from "State Refresh Required", active question locking (`isAnswered={false}, disabled={isSubmitting || isRefreshRequired}`), authoritative durable case synchronization on success, and neutral completion text.
- `frontend/app/(dashboard)/diagnosis/[id]/troubleshooting/page.tsx`: Wired `deriveTroubleshootingView`, `coordinateWorkflowMutation`, `retryWorkflowRefresh`, dedicated error state with retry, distinct refresh warning banners distinguishing "Action Saved" from "State Refresh Required", authoritative durable case synchronization on success, canonical payload submission via `buildCheckResultPayload`, and neutral completion text.
- `frontend/app/(dashboard)/diagnosis/[id]/verification/page.tsx`: Wired independent lifecycle action handlers directly to payload builders, `coordinateWorkflowMutation`, `retryWorkflowRefresh`, revision tracking, dedicated error and distinct refresh warning banners distinguishing "Action Saved" from "State Refresh Required", authoritative durable case synchronization on success, and input preservation.
...
- Added explicit `refreshWarningKind` to distinguish committed POST + failed GET (`"action_saved"`) from unconfirmed POST + failed GET (`"state_refresh_required"`).
- Separated active question input locking (`disabled`) from persisted completion styling (`isAnswered`).

### Verification results

- Task validation: `validate_task.py` passed (VALID).
- Focused backend tests: 89 passed, 25 warnings in 26.65s (`test_case_api.py`, `test_check_result_api.py`, `test_cause_confirmation_api.py`, `test_recovery_verification_api.py`, `test_recurrence_api.py`).
- Frontend workflow regressions: 16/16 tests passed (`node scripts/test-diagnostic-workflow-state.mjs`).
...
`fix(workflow): resolve DLK-M3-029 review findings R11-R12`
