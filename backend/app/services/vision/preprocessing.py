...
class ImageValidationError(ValueError):
    """Client-facing validation error during image preprocessing, decode, or input validation."""
    pass
...
def validate_image_dimensions(
    ...
    if width > max_dim:
        raise ImageValidationError(f"Image width ({width}px) exceeds maximum allowed ({max_dim}px).")
    if height > max_dim:
        raise ImageValidationError(f"Image height ({height}px) exceeds maximum allowed ({max_dim}px).")

    total_pixels = width * height
    if total_pixels > max_pixels:
        raise ImageValidationError(f"Total image pixels ({total_pixels}) exceeds maximum allowed ({max_pixels}).")
...
def decode_and_validate_image(image_bytes: bytes) -> tuple[np.ndarray, ImageDimensions]:
    ...
    if not image_bytes:
        raise ImageValidationError("Empty image buffer.")

    # Validate magic bytes for JPEG or PNG
    is_png = image_bytes.startswith(b"\x89PNG\r\n\x1a\n")
    is_jpeg = image_bytes.startswith(b"\xff\xd8\xff")
    if not (is_png or is_jpeg):
        raise ImageValidationError("Invalid image format: only PNG and JPEG images are supported.")

    np_arr = np.frombuffer(image_bytes, dtype=np.uint8)
    image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

    if image is None:
        raise ImageValidationError("Cannot decode image data; file may be corrupt or truncated.")
