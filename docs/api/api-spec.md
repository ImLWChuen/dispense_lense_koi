...
## 11. Calibrated Image Analysis Pipeline

### 11.1 `POST /api/v1/images/analyze`
Extracts resolution-independent geometric features from dispensing deposit images (camera or inspection captures) and evaluates calibrated diagnostic observations using caller-specified regions of interest (ROIs) and tolerances.

#### Key Architecture & Guarantees
- **Multipart Form-Data Contract:** Uploads binary image bytes via `file`, a JSON specification string via `profile`, and an optional reference image via `reference_file`.
- **Offloaded OpenCV Computation:** CPU-bound computer vision routines (magic byte validation, decoding, ROI segmentation, morphology, and contour analysis) are offloaded to an asynchronous thread worker pool (`anyio.to_thread.run_sync`), keeping the FastAPI event loop fully responsive.
- **Fail-Closed Safety Limits:**
  - File byte limit: Maximum 10 MB per file (enforced before reading payload into memory; returns `413 Payload Too Large`).
  - Decoded dimension limits: Maximum 4096 px per axis and maximum 16,777,216 total decoded pixels (enforced immediately after header decode; returns `422 Unprocessable Content`).
- **Resolution Independence:** All target ROIs are specified in normalized $[0.0, 1.0]$ coordinates. Measurements (coverage ratio, overflow ratio, circularity, solidity, aspect ratio, hole/void ratio) are dimensionless or normalized, yielding identical classifications for proportional deposits across different image resolutions (e.g. 100x100, 200x200, 400x400).
- **Physical Calibration:** No physical units (e.g. millimeters) are guessed or assumed. Physical metrics (`equivalent_diameter_mm`) are calculated if and only if the caller supplies `mm_per_pixel` in the profile.
- **Strictly Bounded Diagnostic Neutrality:**
  - `FEATURES_ONLY` mode produces pure geometric measurements with status `UNCALIBRATED` and zero score-bearing observations.
  - Voids, bubbles, and abnormal shape metrics (D06) are diagnostic-neutral in `FEATURES_ONLY` mode, but generate canonical score-bearing observations when calibrated limits are supplied.
  - Only explicit caller-specified tolerances (`PROCESS_LIMITS` or `REFERENCE_IMAGE`) can generate canonical score-bearing observations.
- **Authoritative Canonical Vocabulary:**
  ...
  - D06: `deposit_shape = tailing`, `deposit_shape = abnormal`, `bubble_presence = visible_bubbles`
- **Lossless Persistence & Provenance:** Observations generated by image analysis use `source = "IMAGE"`, `statement_type = "AI_INFERENCE"`, and preserve all raw ROI measurements in PostgreSQL JSONB `case_observations.metadata` when attached to a durable case.

#### Request Parameters (Multipart Form-Data)

| Form Field | Type | Required | Description |
|---|---|---|---|
| `file` | `UploadFile` (binary) | **Yes** | Image file (JPEG or PNG format, max 10 MB). |
| `profile` | `string` (JSON) | **Yes** | JSON-encoded `AnalysisProfile` object specifying modes, ROIs, and limits. |
| `reference_file` | `UploadFile` (binary) | Optional | Golden reference image file (required when `mode="REFERENCE_IMAGE"`). |

##### Profile JSON Schema (`AnalysisProfile`)
...
- `400 Bad Request`: Missing `reference_file` in `REFERENCE_IMAGE` mode or missing limits object.
- `413 Payload Too Large`: Upload payload exceeds 10 MB limit.
- `422 Unprocessable Content`: Non-image file, corrupt image, dimensions exceeding 4096px, decoded pixel count exceeding 16M pixels, or invalid normalized ROI coordinates.
- `500 Internal Server Error`: Unexpected internal vision failure (sanitized; no paths, internal traces, or partial writes).
...
7. **Calibrated Image Evidence Pipeline:**
   - Upload image files to `POST /api/v1/images/analyze` using `multipart/form-data` with a JSON-serialized `profile` string.
   - Always supply normalized ROIs ($x, y, w, h \in [0.0, 1.0]$) so image measurements remain resolution-independent across sensor resolutions.
   - Attach returned `observations` directly to `POST /api/v1/cases` in the `observations` array. The server losslessly persists all measurement attributes (`metadata` JSONB dict containing coverage, overflow, diameter, circularity, solidity) and exposes them on `CaseObservationResponse.metadata`.
   - Uncalibrated / features-only analysis produces zero score-bearing observations (`observations = []`); callers must provide explicit `process_limits` or a golden `reference_file` to enable calibrated diagnostic observations.
   - D06 bubble and shape features produce score-bearing observations when calibrated shape/bubble process limits or reference tolerances are supplied.
