...
| 10 | `POST /api/v1/cases/{case_id}/recovery-verifications` | `SubmitRecoveryVerificationRequest` &rarr; `CaseRecoveryVerificationResponse` (`200 OK`, `404`, `409`, `422`, `500`) | `casesApi.verifyCase` (`frontend/lib/api/cases.ts:45`) | **Type Mismatch** | **Member 1 Action:**<br>1. Frontend method name is `verifyCase` instead of `submitRecoveryVerification`.<br>2. Frontend return type is typed as `Promise<DiagnosisResult>`, but backend returns `CaseRecoveryVerificationResponse`.<br>3. Frontend maps `status === 'RESOLVED'` to `verification_passed: boolean`, which matches backend logic. `verified_by` defaults to `"technician"` on backend when omitted. |
...
All actor, confirmer, verifier, and reporter string fields enforce max length of 100 characters:
- `confirmed_by`: `str` (max 100)
- `performed_by`: `str` (max 100)
- `verified_by`: `str` (max 100)
- `reported_by`: `str` (max 100)
