...
| 5 | `GET /api/v1/cases/{case_id}` | `DurableCaseResponse` (`200 OK`, `404`, `422`, `500`) | `casesApi.getCase` (`frontend/lib/api/cases.ts`) | **Supported & Wired** | Authority: `DLK-M3-029` & `DLK-M3-030`. Wired in `frontend/app/(dashboard)/cases/[id]/page.tsx`. Hydrates `previous_confirmations` and `lifecycle_events` ordered ascending by revision number for restart-safe resume without engine re-computation. |
